...
if (isset($_GET['id']) && isset($_GET['status'])) {
    ...
    $sql = "UPDATE proyectos SET estado = $status WHERE id = $id";
...
<div class="container mt-5 content-wrapper">
    <div class="card">
        <div class="card-header bg-primary text-white">
            <h2 class="card-title m-0" style="color: white;">Lista de Proyectos</h2>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table id="proyectosTable" class="table table-bordered table-striped table-hover">
                    <thead>
                        <tr class="text-center">
                            <th><i class="fas fa-briefcase"></i> Título</th>
                            <th><i class="fas fa-user"></i> Contratista</th>
                            ...
                            <th><i class="fas fa-calendar"></i> Fecha Inicio</th>
                    ...
                    <tbody>
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo "<tr>
                                    <td>{$row['titulo']}</td>
                                    <td>{$row['contratista_nombre']}</td>
                                    <td>{$row['freelancer_nombre']}</td>
                            ...
                            echo "<tr><td colspan='10' class='text-center'>No se encontraron proyectos</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
...
function toggleStatus(id, status) {
    Swal.fire({
        title: status ? '¿Activar proyecto?' : '¿Desactivar proyecto?',
        icon: 'question',
